...
import semexe.*;
import semexe.Parser.Spec;
import semexe.interactive.DALExecutor;
import semexe.interactive.InteractiveBeamParser;
...
public class FloatingParsingTest {
    ...
    Predicate<Example> contains(String formula) {
        Formula answer = Formulas.formula(LispTree.proto.tree(formula));
        ...
        return new Predicate<Example>() {
            ...
            public boolean test(Example e) {
                ...
                for (Derivation deriv : e.predDerivations) {
                    ...
                    if (required.stream().anyMatch(formula::contains)) {
                        match++;
