...
class InteractiveBeamParserState extends ChartParserState {
    ...
    private boolean canBeRoot(int start, int end) {
        return start == 0 && end == numTokens;
    }
    ...
    protected static void copyDerivs(Map<String, List<Derivation>> source, Map<String, List<Derivation>> dest) {
        if (source == null || dest == null)
            return;

        for (Map.Entry<String, List<Derivation>> stringListEntry : source.entrySet()) {
            List<Derivation> derivations = dest.get((String) stringListEntry.getKey());
            if (derivations == null)
                dest.put((String) stringListEntry.getKey(), derivations = new ArrayList<>());
      ...
      /* add only if the formula not already present to ensure no duplicates */
            ...
            for (Derivation deriv : stringListEntry.getValue()) {
    ...
    protected static void addDerivs(List<Derivation> source, List<Derivation> dest) {
    ...
    private static String catStartEndKey(String cat, int start, int end) {
        return cat + ':' + start + ':' + end;
    }
