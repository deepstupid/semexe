...
public class Aligner {
    ...
    public void saveModel(String out) throws IOException {
        ...
        for (Map.Entry<String, Counter<String>> stringCounterEntry : model.entrySet()) {
            Counter<String> counts = stringCounterEntry.getValue();
            for (String target : counts.keySet()) {
                writer.println(Joiner.on('\t').join((String) stringCounterEntry.getKey(), target, counts.getCount(target)));
            }
        }
    }
