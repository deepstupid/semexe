...
public final class BinaryLexicon {
    ...
    public void updateLexicon(Pair<String, Formula> lexemeFormulaPair, int support) {
        ...
        List<BinaryLexicalEntry> bEntries = MapUtils.get(lexemeToEntryList, lexeme, Collections.emptyList());
        for (BinaryLexicalEntry bEntry : bEntries) {
    ...
    public void sortLexiconByFeedback(Params params) {
        ...
        for (Map.Entry<String, List<BinaryLexicalEntry>> stringListEntry : lexemeToEntryList.entrySet()) {
            Collections.sort(stringListEntry.getValue(), comparator);
            if (opts.verbose > 1) {
                LogInfo.logs("Sorted list for lexeme=%s", (String) stringListEntry.getKey());
                for (BinaryLexicalEntry bEntry : stringListEntry.getValue()) {
    ...
    public static class BinaryLexEntrybyFeaturesComparator implements Comparator<BinaryLexicalEntry> {
    ...
    public static class BinaryLexEntryByCounterComparator implements Comparator<BinaryLexicalEntry> {
