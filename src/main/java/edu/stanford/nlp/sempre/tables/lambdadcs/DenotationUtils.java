...
public final class DenotationUtils {
    ...
    public static Value aggregate(Collection<Value> values, AggregateFormula.Mode mode) {
        ...
        switch (mode) {
            case max:
                return processor.max(values);
            case min:
                return processor.min(values);
            case sum:
                return processor.sum(values);
            case avg:
                return processor.avg(values);
            default:
                throw new LambdaDCSException(Type.invalidFormula, "Unknown aggregate mode: %s", mode);
        }
    }
    ...
    public static UnaryDenotation arithmeticUnary(UnaryDenotation u1, UnaryDenotation u2,
        ...
        switch (mode) {
            case add:
                for (Value v1 : u1) for (Value v2 : u2) answer.add(processor.add(v1, v2));
                break;
            case sub:
                for (Value v1 : u1) for (Value v2 : u2) answer.add(processor.sub(v1, v2));
                break;
            case mul:
                for (Value v1 : u1) for (Value v2 : u2) answer.add(processor.mul(v1, v2));
                break;
            case div:
                for (Value v1 : u1) for (Value v2 : u2) answer.add(processor.div(v1, v2));
                break;
            default:
    ...
    public static UnaryDenotation superlativeUnary(int rank, int count, List<Pair<Value, Value>> pairs,
        ...
        if (LambdaDCSExecutor.opts.superlativesReturnAllTopTies && rank == 1 && count == 1) {
            ...
            switch (mode) {
                case argmax:
                    topValue = processor.max(values);
                    break;
                case argmin:
                    topValue = processor.min(values);
                    break;
                default:
                    throw new LambdaDCSException(Type.invalidFormula, "Unknown superlative mode: %s", mode);
            }
            for (Pair<Value, Value> pair : pairs)
            ...
            switch (mode) {
                case argmax:
                    indices = processor.argsort(values);
    ...
    public abstract static class TypeProcessor {
        ...
        public int compareValues(Value v1, Value v2) {
            throw new LambdaDCSException(Type.typeMismatch, "Cannot compare values with " + getClass().getSimpleName());
        }

        public Value sum(Collection<Value> values) {
            throw new LambdaDCSException(Type.typeMismatch, "Cannot compute sum with " + getClass().getSimpleName());
        }

        public Value avg(Collection<Value> values) {
            throw new LambdaDCSException(Type.typeMismatch, "Cannot compute avg with " + getClass().getSimpleName());
        }

        public Value add(Value v1, Value v2) {
            throw new LambdaDCSException(Type.typeMismatch, "Cannot compute add with " + getClass().getSimpleName());
        }

        public Value sub(Value v1, Value v2) {
            throw new LambdaDCSException(Type.typeMismatch, "Cannot compute sub with " + getClass().getSimpleName());
        }

        public Value mul(Value v1, Value v2) {
            throw new LambdaDCSException(Type.typeMismatch, "Cannot compute mul with " + getClass().getSimpleName());
        }

        public Value div(Value v1, Value v2) {
            throw new LambdaDCSException(Type.typeMismatch, "Cannot compute div with " + getClass().getSimpleName());
        }
