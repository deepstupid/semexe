...
import static semexe.basic.LogInfo.logs;
...
public class FloatingParser extends Parser {
    ...
    protected static void traverseFloatingRules(List<Rule> orderedFloatingRules,
...
class FloatingParserState extends ParserState {
    ...
    private static Object floatingCell(String cat, int depth) {
        return (cat + ':' + depth).intern();
    }

    private static Object anchoredCell(String cat, int start, int end) {
        return (cat + '[' + start + ',' + end + ']').intern();
    }

    private static Object cell(String cat, int start, int end, int depth) {
