...
public class ExecItem extends Item {
    ...
    public static Value getSinceLastModifiedTimeValue(long lastModifiedTime) {
        ...
        return new Value(new StopWatch(time).toString(), String.valueOf(time));
    }
    ...
    public String getStatus() {
        return fileKeyMap.get("output.map:exec.status");
    }
    ...
    protected int getPriority() {
        return Utils.parseIntEasy(fileKeyMap.get("job.map:priority"), 0);
    }

    protected int getReqMemory() {
        return Utils.parseIntEasy(fileKeyMap.get("job.map:reqMemory"), 0);
    }

    protected int getNice() {
        return Utils.parseIntEasy(fileKeyMap.get("job.map:nice"), 0);
    }

    protected String getWorkingDir() {
        return fileKeyMap.get("job.map:workingDir");
    }

    protected String getCommand() {
        return fileKeyMap.get("job.map:command");
    }
