...
public abstract class Item {
    ...
    protected static HtmlElement fieldToCell(Field field, Object value) {
        HtmlElement cell = H.td(String.valueOf(value));
        if (field.numeric) {
    ...
    protected String itemToNewName(OrderedMap<String, Item> items, Item item) {
        if (namesAreIndices()) return String.valueOf(items.size()); // Use numbering
    ...
    protected String itemToNewName(Item item) {
        return itemToNewName(items, item);
    }
    ...
    private static List<String> sortFileNames(List<String> names) {
        ...
        for (String name : names) {
            ...
            pairs.add(new Pair(name, buf + name));
        }
    ...
    protected void updateItemsFromFile(UpdateSpec updateSpec) throws MyException {
        updateItemsFromFile(updateSpec, Collections.emptyList(), Collections.emptyList());
    }
    ...
    private static class Entry implements Comparable<Entry> {
        ...
        public int compareTo(Entry e) {
            return cmpKey.compareTo(e.cmpKey);
        }
    }
}
