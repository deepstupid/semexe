...
public class Grammar {
    ...
    public static BufferedReader open(String path) throws IOException {
        return new BufferedReader(new InputStreamReader(
                Resources.getResource(path).openStream()));
    }
    ...
    public void addStatement(String stmt, String contextPath, Set<String> tags) {
        statements.add(stmt);
        interpret(contextPath, LispTree.proto.tree(stmt), tags.isEmpty() ? opts.tags : Sets.newHashSet(Iterables.concat(tags, opts.tags)));
    }

    public void addStatement(String stmt, String contextPath) {
        Set<String> s = Collections.emptySet();
        addStatement(stmt, contextPath, s);
    }
    ...
    private void readOnePath(String path, Collection<String> tags) {
        ...
        try {
            ...
            //TODO dont read file twice:

            //for (String line : IOUtils.readLinesHard(path))
            BufferedReader a = open(path);
            statements.addAll(IOUtils.readLines(a));
            a.close();

            BufferedReader b = open(path);
            LispTree.proto.parseFromReader(b).forEachRemaining(t -> {
                collectValidTags(interpret(path, t, tags));
            });
            b.close();
    ...
    private List<Rule> binarizeRule(Rule rule, List<Boolean> isOptionals) {
        ...
        if (rule.sem instanceof JoinFn && ((JoinFn) rule.sem).getArg0Fn() != null) {
            ...
            if (j < rule.rhs.size()) {
                ...
                forwardBetaReduce.init(LispTree.proto.tree("(JoinFn forward betaReduce)"));
    ...
    //   (ConstantFn null)
    //   (NumberFn)
    private static SemanticFn parseSemanticFn(LispTree tree) {
    ...
    public static class Options {
        @Option
        public List<String> inPaths = new ArrayList<>();
