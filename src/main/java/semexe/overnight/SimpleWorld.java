...
public final class SimpleWorld {
    ...
    public static List<Value> singleton(Value value) {
    ...
    public static List<Value> countComparative(List<Value> entities, String property, String mode, NumberValue thresholdValue, List<Value> restrictors) {
        ...
        for (Value e : entities) {
            ...
            switch (mode) {
                case "=":
                    if (degree == threshold) newEntities.add(e);
                    break;
                case "<":
                    if (degree < threshold) newEntities.add(e);
                    break;
    ...
    public static List<Value> getProperty(Value inObject, String property) {
    ...
    public static void dumpDatabase() {
        for (Map.Entry<Pair<Value, String>, List<Value>> pairListEntry : database.entrySet())
            LogInfo.logs("%s %s %s", ((Pair<Value, String>) pairListEntry.getKey()).getFirst(), ((Pair<Value, String>) pairListEntry.getKey()).getSecond(), pairListEntry.getValue());
    }
        ...
        public void createEntities(int numEntities) {
            ...
            try {
                new BufferedReader(new InputStreamReader(
                        Resources.getResource(opts.dbPath).openStream()))
                        .lines().forEach(line -> {
