...
public final class Lexicon {
    ...
    private List<LexicalEntry> getCache(String mode, String query) {
        ...
        LispTree tree = LispTree.proto.tree(response);
        List<LexicalEntry> entries = new ArrayList<>();
        for (int i = 0; i < tree.children.size(); i++)
            entries.add(LexicalEntry.LexicalEntrySerializer.entryFromLispTree(tree.child(i)));
        return entries;
    }
    ...
    private void putCache(String mode, String query, List<? extends LexicalEntry> entries) {
        ...
        for (LexicalEntry entry : entries)
            result.addChild(LexicalEntry.LexicalEntrySerializer.entryToLispTree(entry));
        synchronized (cache) {
