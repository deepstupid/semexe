...
public class LambdaCalculusConverter implements Runnable {
    ...
    static void readStringMap(String path, Map<String, String> map) {
    ...
    static boolean isVar(String var) {
    ...
    static String removeGeoType(String pred) {
        return pred.split(":", 2)[0];
    }

    static String getGeoType(String pred) {
    ...
    static boolean validVar(String var, String headVar, List<String> existsVars) {
        if (var == null)
            return false;
        if (!var.startsWith("$"))
            return true;
    ...
    Formula handleExists(LispTree tree, String headVar, List<String> existsVars) {
        ...
        LispTree newPredTree = LispTree.proto.tree(predTree.toString().replace(eVar, newAndTree.toString()));
    ...
    static void countTokens(LispTree tree, Map<String, Integer> counts) {
        if (tree.isLeaf())
        ...
        try {
            ...
            while ((line = in.readLine()) != null) {
                ...
                if (!gotUtterance) {
                    ...
                    LispTree tree = LispTree.proto.tree(preprocessPredicates(line, utterance));
                    LogInfo.logs(Colorizer.colorize("IN [%d]: %s", "blue"), newId, utterance);
                    LogInfo.logs(Colorizer.colorize("IN [%d]: %s", "purple"), newId, tree);
                    ...
                    LogInfo.logs(Colorizer.colorize("OUT [%d]: %s", "yellow"), newId, ex.createExample().targetFormula.toLispTree());
    ...
    void executeExamples() {
        ...
        for (Example ex : examples) {
            ...
            LogInfo.logs(Colorizer.colorize("[%d] %s", "blue"), exInd, ex.utterance);
            LogInfo.logs(Colorizer.colorize("[%d] %s", "yellow"), exInd, ex.targetFormula.toString());
            try {
