...
import semexe.*;
import semexe.freebase.FbFormulasInfo.BinaryFormulaInfo;
import semexe.freebase.FbFormulasInfo.UnaryFormulaInfo;
import semexe.basic.*;
...
public final class FreebaseInfo {
    ...
    private Map<String, String> nameMap = new HashMap<>(); // id => name of id
        ...
        while ((line = in.readLine()) != null) {
            ...
            if (property.equals(NAME) || property.equals(ALIAS)) {
                if (bPopularityMap.containsKey(arg1)) {
                    MapUtils.addToList(bDescriptionsMap, arg1, Utils.parseStr(arg2).toLowerCase());
                } else if (professionPopularityMap.containsKey(arg1)) {
                    MapUtils.addToList(professionDescriptionsMap, arg1, Utils.parseStr(arg2).toLowerCase());
                } else if (typePopularityMap.containsKey(arg1)) {
                    MapUtils.addToList(typeDescriptionsMap, arg1, Utils.parseStr(arg2).toLowerCase());
                }
            }

            if (property.equals(NAME))
                nameMap.put(arg1, Utils.parseStr(arg2));
        }
    ...
    public Map<Formula, BinaryFormulaInfo> createBinaryFormulaInfoMap() {
        ...
        for (Map.Entry<String, Integer> stringIntegerEntry : bPopularityMap.entrySet()) {
            Formula f = Formulas.formula(LispTree.proto.tree((String) stringIntegerEntry.getKey()));
    ...
    public Map<Formula, UnaryFormulaInfo> createUnaryFormulaInfoMap() {
        ...
        Map<Formula, FbFormulasInfo.UnaryFormulaInfo> res = new HashMap<>();
        // professions
        for (Map.Entry<String, Integer> stringIntegerEntry : professionPopularityMap.entrySet()) {
            Formula f = new JoinFormula(PROF, new ValueFormula<Value>(new NameValue((String) stringIntegerEntry.getKey())));
            UnaryFormulaInfo info = new UnaryFormulaInfo(f, stringIntegerEntry.getValue(),
                    MapUtils.get(professionDescriptionsMap, (String) stringIntegerEntry.getKey(), new LinkedList<>()),
        ...
        // types
        for (Map.Entry<String, Integer> stringIntegerEntry : typePopularityMap.entrySet()) {
            Formula f = new JoinFormula(TYPE, new ValueFormula<Value>(new NameValue((String) stringIntegerEntry.getKey())));
            UnaryFormulaInfo info = new UnaryFormulaInfo(f, stringIntegerEntry.getValue(),
                    MapUtils.get(typeDescriptionsMap, (String) stringIntegerEntry.getKey(), new LinkedList<>()),
                    Collections.singleton((String) stringIntegerEntry.getKey()));
            if (!info.isComplete()) {
                continue;
            }
            res.put(f, info);
    ...
    public static class Options {
        ...
        public String schemaPath = "fb_data/93.exec/schema2.ttl";
    }
}
