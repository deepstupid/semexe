...
public final class FbFormulasInfo {
    ...
    private static boolean isLegalCvt(Formula inner, Formula outer) {
        if (FreebaseInfo.isReverseProperty(inner.toString()) && !FreebaseInfo.isReverseProperty(outer.toString()))
            return false;
        return FreebaseInfo.isReverseProperty(inner.toString()) || !FreebaseInfo.isReverseProperty(outer.toString());
    }
    ...
    public boolean hasOpposite(String formula) {
        return hasOpposite(LispTree.proto.tree(formula));
    }
     ...
     * supports chains only
     */
    public static boolean isReversed(Formula formula) {
    ...
    public Formula equivalentFormula(String formula) {
        LispTree tree = LispTree.proto.tree(formula);
    ...
    public static class BinaryFormulaInfo {
        ...
        public boolean isComplete() {
            return formula != null && expectedType1 != null && expectedType2 != null &&
                    !expectedType1.isEmpty() && !expectedType2.isEmpty() && descriptions != null &&
                    descriptions.size() != 0 &&
                    !(popularity == 0.0);
        }
        ...
        public static String extractDomain(Formula binary) {
    ...
    public static class UnaryFormulaInfo {
        ...
        public boolean isComplete() {
            return formula != null && descriptions != null && descriptions.size() != 0 &&
                    !(popularity == 0.0);
        }
