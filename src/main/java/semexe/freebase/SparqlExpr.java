...
class SparqlBlock implements SparqlExpr {
    ...
    private static String getId(PrimitiveFormula formula) {
    ...
    private static boolean isPrimitiveType(String id) {
        ...
        return FreebaseInfo.TEXT.equals(id);
    }
...
class SparqlSelect implements SparqlExpr {
    ...
    public String toString() {
        ...
        for (Var var : selectVars)
            out.append(' ').append(var.toString());
        out.append(" WHERE ").append(where);
        if (sortVars.size() > 0) {
            out.append(" ORDER BY");
            for (PrimitiveFormula sortVar : sortVars)
                out.append(' ').append(SparqlUtils.plainStr(sortVar));
        }
        if (limit != -1) out.append(" LIMIT ").append(limit);
...
class SparqlStatement implements SparqlExpr {
    ...
    private static DateValue advance(DateValue date) {
...
final class SparqlUtils {
    ...
    public static String plainStr(PrimitiveFormula formula) {
        ...
        if (value instanceof NumberValue) return String.valueOf(((NumberValue) value).value);
