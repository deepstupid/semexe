...
public class FilterFreebase implements Runnable {
    ...
    public HashSet<String> schemaProperties = new HashSet<>(
            ImmutableList.of(
    ...
    public HashSet<String> generalProperties = new HashSet<>(
            ImmutableList.of(
                    "fb:type.object.type",
                    "fb:type.object.name",
    ...
    private void readKeep() {
        ...
        for (String path : keepTypesPaths)
            keepTypes.addAll(IOUtils.readLinesHard(path));

        // Keep and not keep properties
        for (String path : keepPropertiesPaths)
            keepProperties.addAll(IOUtils.readLinesHard(path));
    ...
    private void filterTuples() {
        ...
        TDoubleMap<String> propertyCounts = new TDoubleMap<>();

        PrintWriter out = IOUtils.openOutHard(Execution.getFile("0.ttl"));
        out.println(semexe.freebase.Utils.ttlPrefix);
        ...
        try {
            ...
            while (numInputLines < maxInputLines && (line = in.readLine()) != null) {
                ...
                String[] tokens = semexe.freebase.Utils.parseTriple(line);
                ...
                if (semexe.freebase.Utils.isUrl(arg2)) continue;
                if (semexe.freebase.Utils.identifierContainsStrangeCharacters(arg1) || semexe.freebase.Utils.identifierContainsStrangeCharacters(arg2))
                    continue;

                semexe.freebase.Utils.writeTriple(out, arg1, property, arg2);
