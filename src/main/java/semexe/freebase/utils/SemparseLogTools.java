...
public final class SemparseLogTools {
    ...
    private static void getFbDescInTrueDerivations(String log, String out) throws IOException {
        ...
        for (String line : IOUtils.readLines(log)) {
            ...
            if (start) {
                ...
                if (line.contains("True@")) {
                    ...
                    LispTree t = LispTree.proto.tree(formula);
        ...
        for (Map.Entry<String, Set<String>> stringSetEntry : exampleToDescriptions.entrySet()) {
            if (stringSetEntry.getValue().size() > 0)
                writer.println(stringSetEntry.getKey() + '\t' + stringSetEntry.getValue());
        }
        writer.close();
    }
        ...
        for (String line : IOUtils.readLines(log)) {
            ...
            if (start) {
                ...
                if (line.contains("Current:")) {
                    if (field.equals("correct")) {
                        print = line.contains("correct=0") && line.contains("oracle=1");
                    }
                    if (field.equals("oracle")) {
                        print = line.contains("oracle=0") && line.contains("parsed=1");
                    }
                    if (field.equals("parsed")) {
                        print = line.contains("parsed=0");
                    }
                }
            }
        }
