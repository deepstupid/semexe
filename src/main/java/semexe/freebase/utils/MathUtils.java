...
public final class MathUtils {
    ...
    public static <T> void normalizeDoubleMap(Map<T, DoubleContainer> map) {
        ...
        for (DoubleContainer d : map.values())
            sum += d.value();
        for (Map.Entry<T, DoubleContainer> tDoubleContainerEntry : map.entrySet()) {
            double normalizedValue = tDoubleContainerEntry.getValue().value() / sum;
            tDoubleContainerEntry.getValue().set(normalizedValue);
        }
    }
