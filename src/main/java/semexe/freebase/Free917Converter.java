...
import semexe.freebase.utils.FileUtils;
import semexe.freebase.utils.FormatConverter;
...
import semexe.prob.SampleUtils;
import semexe.*;
...
public class Free917Converter implements Runnable {
    ...
    private static void correctErrors(String inQuestionsFile, String inNpFile,
    ...
    private static void printToFile(String fileName, List<String> examples) throws IOException {
    ...
    private Formula processFree917LogicalForm(String free917LogicalForm) {
        ...
        LispTree tree = LispTree.proto.tree(free917LogicalForm);
    ...
    private Formula generateFormula(Map<String, String> argToPredMap) {
        ...
        if (argToPredMap.size() == 1) {
            ...
            FbFormulasInfo.BinaryFormulaInfo info = formulaInfo.getBinaryInfo(Formulas.formula(LispTree.proto.tree(pred)));
            if (info != null) {
                String type = formulaInfo.getBinaryInfo(Formulas.formula(LispTree.proto.tree(pred))).expectedType1;
    ...
    private static Formula fixCvtFormulas(String pred, String arg) {
    ...
    private Formula conjunctArgs(Map<String, String> argToPredMap) {
        ...
        List<JoinFormula> pivots = new ArrayList<>();
        for (Map.Entry<String, String> stringStringEntry : argToPredMap.entrySet()) {
            if (!((String) stringStringEntry.getKey()).equals("target"))
                pivots.add(constructJoin((String) stringStringEntry.getKey(), stringStringEntry.getValue()));
        }
        Formula res = pivots.get(0);
        if (pivots.size() == 1) return res;
        for (int i = 1; i < pivots.size(); ++i)
            res = new MergeFormula(MergeFormula.Mode.and, res, pivots.get(i));
        return res;
    }

    private static Formula getArgFormula(String arg) {
    ...
    private static JoinFormula constructJoin(String arg, String pred) {
    ...
    private static List<String> constructFbArguments(LispTree tree) {
    ...
    private static List<String> constructFbRelations(String[] predTokens, String fbType) {
    ...
    private static String constructFbType(String predHead) {
     ...
     */
    private static Formula handleAsk(LispTree tree) {
        ...
        MergeFormula mf = new MergeFormula(MergeFormula.Mode.and, arg1, pf);
        return mf;
    }
    ...
    public void genreateEntityInfoFile(String free917EntityFile, String entityInfoFile, String outFile, String missingEntitiesFile) throws IOException {
        ...
        Map<String, Set<String>> idToNameMap = new HashMap<>();
        ...
        for (Map.Entry<String, Set<String>> stringSetEntry : idToNameMap.entrySet()) {
            if (((String) stringSetEntry.getKey()).startsWith("fb:type") || ((String) stringSetEntry.getKey()).startsWith("fb:un."))
                continue;
            missingWriter.println(stringSetEntry.getKey() + '\t' + stringSetEntry.getValue());
        }
        missingWriter.close();
    }
}
