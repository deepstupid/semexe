...
public abstract class Parser {
    ...
    protected static void traverse(List<Rule> catUnaryRules, String node,
     ...
     */
    public static void addToEvaluation(ParserState state, Evaluation evaluation) {
        Example ex = state.ex;
        ...
        for (int i = 0; i < predDerivations.size(); i++) {
            ...
            if (compatibilities != null && compatibilities[i] == 1) {
                ...
                if (print) {
                    ...
                    numPrintedSoFar++;
                    if (opts.dumpAllFeatures)
                        FeatureVector.logFeatureWeights("Features", deriv.getAllFeatureVector(), state.params);
                }
            }
        }
        // Partially correct
