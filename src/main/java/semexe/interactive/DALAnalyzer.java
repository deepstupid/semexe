...
import semexe.LanguageAnalyzer;
...
public class DALAnalyzer extends LanguageAnalyzer {
    ...
    private static boolean possiblyNumber(String token) {
        return !token.isEmpty() && !(Character.isLetter(token.charAt(0))); //hack for performance
    }

    private static void parseNonNumber(LanguageInfo languageInfo, String token) {
    ...
    public LanguageInfo analyze(String utterance) {
        ...
        if (!utterance.isEmpty()) {
            ...
            for (String token : tokens) {
                ...
                int x = ArrayUtils.indexOf(numbers, token);
                if (x != -1) {
                    languageInfo.posTags.add("CD");
                    languageInfo.nerTags.add("NUMBER");
                    languageInfo.nerValues.add(String.valueOf(x));
                    continue;
                }

                if (possiblyNumber(token)) {
                    try {
                        ...
                        parseNonNumber(languageInfo, token);
                    }
                } else {
                    parseNonNumber(languageInfo, token);
                }
