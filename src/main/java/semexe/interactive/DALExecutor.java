...
public class DALExecutor extends Executor {
    ...
    private static Object toObject(Value value) {
        if (value instanceof NumberValue && opts.convertNumberValues) {
            ...
            List<Value> vv = ((ListValue) value).values;
            List<Object> list = new ArrayList(vv.size());
            for (Value elem : vv)
    ...
    private static Set<Object> toSet(Object maybeSet) {
    ...
    private static Object toElement(Set<Object> set) {
        if (set.size() == 1) {
    ...
    private static Set<Item> toItemSet(Set<Object> maybeItems) {
