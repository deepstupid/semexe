...
public class DALFeatureComputer implements FeatureComputer {
    ...
    private static List<String> abstractAnchors(Derivation deriv, List<String> tokens, int window) {
        if (deriv.start == -1)
            return tokens;
    ...
    private static List<String> getAllNgrams(List<String> tokens, int n, Derivation deriv) {
    ...
    private static List<String> getAllSkipGrams(List<String> tokens, Derivation deriv) {
        List<String> ngrams = new ArrayList<>();
        List<String> paddedTokens = new ArrayList<>();
        if (tokens.size() < 3)
            return ngrams;
        ...
        for (int i = 0; i < tokens.size() - 2; i++) {
            ngrams.add('[' + tokens.get(i) + ", *, " + tokens.get(i + 2) + ']');
        }
        return ngrams;
    }

    private static void addWindowFeatures(Example ex, Derivation deriv) {
    ...
    private static void addStatsFeatures(Example ex, Derivation deriv) {
    ...
    private static void addSocialFeatures(Example ex, Derivation deriv) {
    ...
    private static void extractRuleFeatures(Example ex, Derivation deriv) {
    ...
    private static void extractSpanFeatures(Example ex, Derivation deriv) {
