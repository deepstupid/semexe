...
public class InteractiveMaster extends Master {
    ...
    public static List<Rule> induceRulesHelper(String command, String head, String jsonDef, Parser parser, Params params,
        ...
        List<Rule> inducedRules = new GrammarInducer(exHead.getTokens(), bodyDeriv, state.chartList).getRules();
        for (int i = 0, inducedRulesSize = inducedRules.size(); i < inducedRulesSize; i++) {
            Rule rule = inducedRules.get(i);
    ...
    void handleCommand(Session session, String line, Response response) {
        LispTree tree = LispTree.proto.tree(line);
        ...
        if (command.equals(":q")) {
            ...
            try {
                targetFormulas = tree.children.subList(2, tree.children.size()).stream()
                        .map(t -> Formulas.formula(LispTree.proto.tree(t.value))).collect(Collectors.toList());
            } catch (Exception e) {
                e.printStackTrace();
