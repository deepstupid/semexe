...
import static semexe.Grammar.open;
...
public abstract class AbstractLispTree<TreeType extends AbstractLispTree> implements MemUsage.Instrumented {
    ...
    public TreeType addChild(TreeType tree) {
        children.add(tree);
        return (TreeType) this;
    }

    public TreeType addChild(String value) {
        addChild(newLeaf(value));
        return (TreeType) this;
    }
    ...
    public <K, V> TreeType newList(Map<K, V> items) {
        TreeType tree = newList();
        for (Map.Entry<K, V> e : items.entrySet()) tree.addChild(newList(String.valueOf(e.getKey()), String.valueOf(e.getValue())));
        return tree;
    }

    public <T> TreeType newList(List<T> items) {
        TreeType tree = newList();
        for (T x : items) tree.addChild(String.valueOf(x));
        return tree;
    }
    ...
    public Iterator<TreeType> parseFromFile(String path) {
        try {
            return parseFromReader(IOUtils.openIn(path));
        } catch (IOException e) {
            try {
                return parseFromReader(open(path));
    ...
    public Iterator<TreeType> parseFromReader(BufferedReader in) {
        return new ParseLispTreeIterator(in, this);
    }

    public TreeType tree(String str) {
    ...
    public String toString() {
        return toStringWrap(Integer.MAX_VALUE);
    }

    public String toStringWrap() {
        return toStringWrap(defaultMaxWidth);
    }

    public String toStringWrap(int maxWidth) {
        return toStringWrap(maxWidth, maxWidth);
    }  // Wrap at maxWidth
    ...
    public void print(Writer out) {
        print(defaultMaxWidth, defaultMaxWidth, out);
    }
            ...
            if (c == 0)  // Nothing
                ...
                while (c != 0) {
                    if (escaped) {
                        ...
                        if (!in_quote && (Character.isWhitespace(c) || c == ')'))
                            break;
                        value.append(c);
                    }
                    advance();
