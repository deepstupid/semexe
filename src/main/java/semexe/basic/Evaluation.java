...
public class Evaluation {
    ...
    private List<StatFig> values = new ArrayList<>();

    // Just for printing out
    private Set<String> cumulativeNames = new HashSet<>();
    ...
    public void addCumulative(String name, boolean value) {
        addCumulative(name, value ? 1 : 0);
    }
    ...
    public String summary(String delim) {
        ...
        for (int i = 0; i < names.size(); i++) {
            ...
            out.append(names.get(i)).append('=').append(Fmt.D(value));
        }
        return out.toString();
    }

    private static void putOutput(String prefix, StatFig fig) {
    ...
    private static String basePrefix(String prefix) {
    ...
    Map<String, Map<String, Object>> toMap() {
        Map<String, Map<String, Object>> m = new HashMap<>(names.size());
        for (int i = 0; i < names.size(); i++)
