...
public class StringDoubleMap implements Iterable<StringDoubleMap.Entry>, Serializable {
    ...
    static void check(Set<Integer> set, StringDoubleMap map) {
        for (int x : set) {
            double value = map.getSure(String.valueOf(x));
            assert value == 1.0 * x;
        }
    ...
    static StringDoubleMap test(int n, int range) {
        ...
        for (int x : set) {
            map.put(String.valueOf(x), 1.0 * x);
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args[0].equals("ser")) {
            ...
            HashMap<String, Double> omap = new HashMap<>();
            for (int q = 0; q < T; q++) {
                for (int i = 0; i < n; i++) {
    ...
    public StringDoubleMap copy() {
        ...
        newMap.num = num;
        newMap.keys = locked ? keys : keys.clone(); // Share keys!
    ...
    private void repCheck() {
        ...
        if (mapType == MapType.SORTED_LIST) {
            ...
            for (int i = 1; i < num; i++) { // Make sure keys are sorted.
                ...
                int h2 = hash(keys[i]);
                assert h1 <= h2;
                assert h1 != h2 || keys[i - 1].compareTo(keys[i]) < 0;
            }
        }
    }
