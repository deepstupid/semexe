...
public class NumUtils {
    ...
    public static boolean isFinite(double[] xs) {
        for (double x : xs) if (!isFinite(x)) return false;
        return true;
    }
    ...
    public static void assertIsFinite(double[] xs) {
        for (double x : xs) assert isFinite(x) : "Not finite: " + Fmt.D(xs);
    }

    public static void assertIsFinite(double[][] xss) {
        for (double[] xs : xss) for (double x : xs) assert isFinite(x) : "Not finite: " + Fmt.D(xss);
    }
    ...
    public static double[][] copy(double[][] mat) {
        ...
        for (int r = 0; r < m; r++) {
            ...
            System.arraycopy(mat[r], 0, newMat[r], 0, n);
        }
        return newMat;
    }
