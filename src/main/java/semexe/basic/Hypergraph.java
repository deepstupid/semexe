...
public class Hypergraph<Widget> {
    ...
    private final NullHyperedgeInfo<Widget> nullHyperedgeInfo = new NullHyperedgeInfo<>();
    ...
    public void addEdge(Object source, AbstractHyperedgeInfo<Widget> info) {
    ...
    public void addEdge(Object source, Object dest1, AbstractHyperedgeInfo<Widget> info) {
    ...
    private void computeTopologicalOrdering() {
        ...
        assert allowEmptyNodes || topologicalOrdering[topologicalOrdering.length - 1] == endNodeInfo;
    }
                ...
                case sum:
                    for (Hyperedge edge : nodeInfo.edges) {
                        if (edge.dest1 != endNodeInfo)
                            edge.dest1.outsideScore.incr_mult3(nodeInfo.outsideScore, edge.weight, edge.dest2.insideScore);
                        if (edge.dest2 != endNodeInfo)
                            edge.dest2.outsideScore.incr_mult3(nodeInfo.outsideScore, edge.weight, edge.dest1.insideScore);
                    }
                    break;
                case prod:
                    for (Hyperedge edge : nodeInfo.edges) {
                        if (edge.dest1 != endNodeInfo)
                            edge.dest1.outsideScore.incr_mult2div1(nodeInfo.outsideScore, nodeInfo.insideScore, edge.dest1.insideScore);
                        if (edge.dest2 != endNodeInfo)
                            edge.dest2.outsideScore.incr_mult2div1(nodeInfo.outsideScore, nodeInfo.insideScore, edge.dest2.insideScore);
                    }
                    break;
            }
        }
    ...
    public enum NodeType {prod, sum}

    private interface AbstractHyperedgeInfo<Widget> {
        void setPosterior(double prob);

        Widget choose(Widget widget); // Return the updated widget
    }
    ...
    private static class Hyperedge {
        ...
        public String toString() {
            return String.format("%s %s (%s)", dest1, dest2, weight);
        }
    }
    ...
    private static class HyperedgeTest {
        @Option(gloss = "Number of possible values of the latent variable")
        public int K = 3;
        @Option(gloss = "Number of possible values of observed variables")
        public int D = 3;
        @Option(gloss = "Number of observed variables")
        public int L = 3;
        @Option(gloss = "Number of examples to generate")
        public int numExamples = 100;
        @Option(gloss = "Number of EM iterations to run")
        public int numIters = 20;
    ...
    private static class NullHyperedgeInfo<Widget> implements HyperedgeInfo<Widget> {
        public double getWeight() {
            return 1;
        }
