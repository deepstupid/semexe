...
public class BipartiteMatcher {
    ...
    public static int[] findMaxWeightAssignment(double[][] matrix) {
        matrix = copy(matrix);
        for (int i = 0; i < matrix.length; i++)
    ...
    public static int[] findMinWeightAssignment(double[][] matrix) {
        matrix = copy(matrix);
        return findBestAssignment(matrix);
    }
    ...
    private static int[] findBestAssignment(double[][] matrix) {
    ...
    private static void incrementSetOfStarredZeroes(int[] unpairedZeroPrime, int[] starsByRow, int[] starsByCol, int[] primesByRow) {
        ...
        Set<int[]> zeroSequence = new LinkedHashSet<>();
