...
public class ListUtils {
    ...
    public static <T> T[][] shallowClone(T[][] v) {
        ...
        for (int i = 0; i < v.length; i++) {
            newv[i] = newArray(v[i]);
            System.arraycopy(v[i], 0, newv[i], 0, v[i].length);
        }
        return newv;
    }
    ...
    public static double[][] set(double[][] v, double[][] x) {
        for (int i = 0; i < v.length; i++)
            System.arraycopy(x[i], 0, v[i], 0, v[i].length);
        return v;
    }
    ...
    public static double[] concat(double[] v1, double[] v2) {
        ...
        System.arraycopy(v2, 0, v, v1.length + 0, v2.length);
        return v;
    }
    ...
    public static String[] subArray(String[] v, int start, int end) {
        ...
        System.arraycopy(v, start, subv, start - start, end - start);
        return subv;
    }
    ...
    public static int[] sortedIndices(double[] list, int n, boolean reverse) {
        // Sort
        List<Pair<Double, Integer>> pairList = new ArrayList<>(n);
        for (int i = 0; i < n; i++)
            pairList.add(new Pair<>(list[i], i));
    ...
    public static int[] sortedIndices(int[] list, boolean reverse) {
        int n = list.length;
        // Sort
        List<Pair<Integer, Integer>> pairList = new ArrayList<>(n);
        for (int i = 0; i < n; i++)
            pairList.add(new Pair<>(list[i], i));
        Collections.sort(pairList,
                reverse ? new Pair.ReverseFirstComparator<>()
