...
import static semexe.basic.LogInfo.errors;
...
public class TDoubleMap<T> extends AbstractTMap<T>
    ...
    public TDoubleMap(AbstractTMap<T> map) {
        ...
        this.num = map.num;
        this.keys = map.locked ? map.keys : map.keys.clone(); // Share keys! CHECKED
    ...
    public TDoubleMap<T> copy() {
        TDoubleMap<T> newMap = new TDoubleMap<>(keyFunc);
        newMap.mapType = mapType;
        newMap.locked = locked;
        newMap.num = num;
        newMap.keys = locked ? keys : keys.clone(); // Share keys! CHECKED
    ...
    private void switchMapType(MapType newMapType) {
        ...
        if (newMapType == MapType.SORTED_LIST) {
            // Sort the keys
            List<FullEntry> entries = new ArrayList<>(num);
    ...
    private void repCheck() {
        ...
        if (mapType == MapType.SORTED_LIST) {
            ...
            for (int i = 1; i < num; i++) { // Make sure keys are sorted.
                ...
                int h2 = hash(keys[i]);
                assert h1 <= h2;
                assert h1 != h2 || ((Comparable) keys[i - 1]).compareTo(keys[i]) < 0;
            }
        }
    }
