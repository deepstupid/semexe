...
public class ReinforcementParser extends Parser {
    ...
    private static void addToSiblingMap(String keySibling, String valueSibling, Rule rule,
...
final class ReinforcementParserState extends AbstractReinforcementParserState {
    ...
    private static double computeExpectedReward(List<Derivation> predDerivations, double[] probs) {
    ...
    private void computeGradient() {
        ...
        if (ReinforcementParser.opts.simulateNonRlObjective) {
            computeExpectedCounts(predDerivations, sampleCounts);
        } else {
        ...
        double sum = 0d;
        for (Map.Entry<String, Double> stringDoubleEntry : sampleCounts.entrySet()) {
            double value = stringDoubleEntry.getValue();
            if (Parser.verbose(3))
                LogInfo.logs("feature=%s, value=%s", (String) stringDoubleEntry.getKey(), value);
            sum += value * value;
        }
    ...
    public static class CorrectDerivationComparator implements Comparator<Derivation> {
        ...
        private static boolean containsJoin(Derivation d) {
