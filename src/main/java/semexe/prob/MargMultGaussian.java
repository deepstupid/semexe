...
public class MargMultGaussian implements MargDistrib<NormalInverseWishart> {
    ...
    public double margLogLikelihood(SuffStats stats) {
        ...
        sum += MultGaussian.getStdNormal(dim()).logProb(stats); // p(x | theta = a)
        sum -= getPosterior(stats).meanVarDistrib.unNormalizedLogProb(MultGaussian.getZeroVector(dim()), MultGaussian.getIdentityMtx(dim())); // p(theta = a | x)
        return sum;
    }
    ...
    public double predLogLikelihood(SuffStats condStats, SuffStats predStats) {
        return getPosterior(condStats).margLogLikelihood(predStats);
    }
