...
public class FuncCommandNode extends TwigRecordNode implements CommandNode {
    public static final CommandNode noOpCmd =
            new FuncCommandNode("noOp", Collections.emptyList(), null, false);
    public static final CommandNode identityCmd =
            new FuncCommandNode("identity", Collections.emptyList(), null, false);
    ...
    public static final CommandNode keySkeletonCmd =
            new FuncCommandNode("keySkeleton", Collections.emptyList(), identityCmd, false);
    public static final CommandNode withoutChildrenCmd =
            new FuncCommandNode("withoutChildren", Collections.emptyList(), identityCmd, false);
    ...
    private static RecordNode extractKeySkeleton(RecordNode record,
        ...
        if (maxDepth > 0) {
            ...
            for (RecordNode childRecord : record.getChildren()) {
                ...
                result.addChild(extractKeySkeleton(childRecord,
                        maxDepth - 1, childKey, String.valueOf(counts.get(childKey)), numNodes));
    ...
    private class ExecState {
        ...
        public RecordNode exec() {
            ...
            if (name.equals("sub") || name.equals("head") || name.equals("tail") ||
                ...
                if (lname.equals("sub")) {
                    ...
                    hint = new PeriodicSubsetHint(
                            ListUtils.newList("" + 0, String.valueOf(parser.getInt("count", 10))), bindings);
                } else if (lname.equals("tail")) {
                    parser.setNames("count").parse(bindings);
                    hint = new PeriodicSubsetHint(
                            ListUtils.newList(String.valueOf(-parser.getInt("count", 10)), "1f"), bindings);
                } else
            ...
            if (name.equals("define")) {
                ...
                newResult = RecordNodeUtils.prependKeyValue(result, "diff", String.valueOf(diff));
                ...
                if (type != null && result instanceof PathRecordNode) {
                    ...
                    key = type;
                    if (type.equals("mtime")) value = String.valueOf(path.lastModified());
                    else if (type.equals("path")) value = path.toString();
                    else if (type.equals("size")) value = String.valueOf(path.length());
                }
                ...
                if (name.equals("source")) {
                    ...
                    for (String line : IOUtils.readProgramLinesHard(path)) {
                        ...
                        if (detail) newFullResult.addChild(child.shallowCopy("line", String.valueOf(lineNum)));
                    }
                    ...
                    newFullResult.addChild(new LeafRecordNode("count", String.valueOf(fig.count())));
                    newFullResult.addChild(new LeafRecordNode("min", String.valueOf(fig.min())));
                    newFullResult.addChild(new LeafRecordNode("max", String.valueOf(fig.max())));
                    newFullResult.addChild(new LeafRecordNode("mean", String.valueOf(fig.mean())));
                    newFullResult.addChild(new LeafRecordNode("stddev", String.valueOf(fig.stddev())));
                    newFullResult.addChild(new LeafRecordNode("sum", String.valueOf(fig.sum())));
                    ...
                    if (bestNode != null)
                        newFullResult.addChild(RecordNodeUtils.appendKeyValue(bestNode, "index", String.valueOf(bestIndex)));
                    ...
                    newFullResult.addChild(new LeafRecordNode(name, String.valueOf(numGoodPoints)));
                    ...
                    for (int i = 0; i < children.size(); i++) {
                        ...
                        if (i - window >= 0) { // Remove
                            ...
                            if (NumUtils.isFinite(oldx)) {
                                sum -= oldx;
                                n--;
                            }
                        }
                        newFullResult.addChild(node.shallowCopy(node.getKey(), String.valueOf(sum / n)));
                    }
                    ...
                    List<Pair<Double, RecordNode>> pairs =
                            new ArrayList<>();
                    for (RecordNode node : result.getChildren())
                        pairs.add(new Pair<>(node.getDoubleValue(), node));
                    ListUtils.partialSort(pairs, n,
                            reverse ? new Pair.ReverseFirstComparator<>()
                                    : new Pair.FirstComparator<>());
