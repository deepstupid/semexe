...
public class ApplyFn extends SemanticFn {
    ...
    private static Formula hackJoin(Formula formula) {
    ...
    public Collection<ChildDerivationsGroup> getFilteredDerivations(List<Derivation> derivations1, List<Derivation> derivations2) {
        ...
        if (sameType) {       // Matching pairs
            for (Map.Entry<String, List<Derivation>> stringListEntry : grouped1.entrySet()) {
                if (!grouped2.containsKey((String) stringListEntry.getKey())) continue;
                groups.add(new ChildDerivationsGroup(stringListEntry.getValue(), grouped2.get((String) stringListEntry.getKey())));
            }
    ...
    private static Map<String, List<Derivation>> groupByType(List<Derivation> derivations, SemType parentType) {
