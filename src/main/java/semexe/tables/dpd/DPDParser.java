package semexe.tables.dpd;


import semexe.tables.DenotationTypeInference;
import semexe.tables.InfiniteListValue;
import semexe.tables.ScopedValue;
import semexe.tables.TableDerivationPruningComputer;
import semexe.tables.grow.ApplyFn;
import semexe.basic.*;
import semexe.*;
...
public class DPDParser extends FloatingParser {
    ...
    public enum DumpSpec {NONE, UNIQUE, NONERROR, ALL, FORMULA}
...
class DPDParserState extends ParserState {
    ...
    private static String anchoredCell(String cat, int start, int end) {
        return (cat + '[' + start + ',' + end + ']').intern();
    }

    private static String floatingCell(String cat, int depth) {
    ...
    private static boolean isRootRule(Rule rule) {
    ...
    private void collectPossibleIngredients(String cell, Set<BackPointer> usedBps) {
        ...
        for (Map.Entry<Value, Metadata> valueMetadataEntry : denotationToMetadata.entrySet()) {
            double compatibility = parser.valueEvaluator.getCompatibility(ex.targetValue, (Value) valueMetadataEntry.getKey());
            if (compatibility != 1) continue;
            if (Parser.opts.verbose >= 2)
                LogInfo.logs("[%f] %s", compatibility, valueMetadataEntry.getValue().derivations.get(0));
            BackPointer bp = new BackPointer(cell, (Value) valueMetadataEntry.getKey());
    ...
    private Map<String, Integer> countNumCells(Map<String, Map<Value, Metadata>> cells) {
        ...
        for (Map.Entry<String, Map<Value, Metadata>> entry : cells.entrySet()) {
            if (entry.getKey().contains(",")) numAnchored++;
            else numFloating++;
    ...
    protected static void classifyUniqueDenotations(Map<String, Map<Value, Metadata>> cells) {
    ...
    protected static String getDenotationAttributes(Value denotation) {
        ...
        if (denotation instanceof ListValue || denotation instanceof InfiniteListValue) {
            ...
            sb.append("X=").append(denotation.getClass().getSimpleName());
        }
        return sb.toString();
    }
    ...
    class Ingredient {
        ...
        public boolean equals(Object o) {
            ...
            if ((child1 == null && that.child1 != null) || (child1 != null && !child1.equals(that.child1)))
                return false;
            return (child2 != null || that.child2 == null) && (child2 == null || child2.equals(that.child2));
        }
