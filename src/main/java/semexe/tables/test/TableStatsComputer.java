...
import semexe.tables.TableTypeSystem;
import semexe.tables.test.CustomExample.ExampleProcessor;
import semexe.basic.*;
...
public class TableStatsComputer implements Runnable {
    ...
    static class TableStatsComputerProcessor implements ExampleProcessor {
        ...
        public void run(CustomExample ex) {
            ...
            outputFields.add(String.valueOf(graph.numRows()));
            outputFields.add(String.valueOf(graph.numColumns()));
            outputFields.add(String.valueOf(graph.numUniqueCells()));
            ...
            if (value instanceof DescriptionValue) {
                ...
                Formula formula = new MergeFormula(Mode.and,
                        ...
                        new JoinFormula(Formula.fromString(TableTypeSystem.CELL_NUMBER_VALUE.id), new ValueFormula<>(value)));
                ...
                Formula formula = new MergeFormula(Mode.and,
                        new JoinFormula(Formula.fromString(CanonicalNames.TYPE), Formula.fromString(TableTypeSystem.CELL_GENERIC_TYPE)),
                        new JoinFormula(Formula.fromString(TableTypeSystem.CELL_DATE_VALUE.id), new ValueFormula<>(value)));
                Value result = builder.executor.execute(formula, ex.context).value;
                inTable = result instanceof ListValue && !((ListValue) result).values.isEmpty();
                evaluation.add("value-number-in-table", inTable);
            } else {
            ...
            outputFields.add(String.valueOf(inTable));
        ...
        static void addIfOK(String x, Map<String, Integer> collection) {
        ...
        static void dumpCollection(Map<String, Integer> collection, String filename) {
