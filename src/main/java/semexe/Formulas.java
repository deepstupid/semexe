...
public abstract class Formulas {
    ...
    public static Formula formula(LispTree tree) {
        ...
        { // ActionFormula
            ...
            if (mode != null) {
                List<Formula> args = IntStream.range(1, s).mapToObj(i -> formula(tree.child(i))).collect(Collectors.toList());
    ...
    public static Formula substituteVar(Formula formula, final String var, final Formula replaceFormula) {
        return formula.map(
                f -> {
                    if (f instanceof VariableFormula) {  // Replace variable
                        String name = ((VariableFormula) f).name;
                        return var.equals(name) ? replaceFormula : f;
                    } else if (f instanceof LambdaFormula) {
                        if (((LambdaFormula) f).var.equals(var)) // |var| is bound, so don't substitute inside
                            return f;
                    }
                    return null;
                });
        ...
        return formula.map(
                formula1 -> {
                    if (formula1 instanceof JoinFormula) {
                        Formula relation = betaReduction(((JoinFormula) formula1).relation);
                        Formula child = ((JoinFormula) formula1).child;
